...
import static com.zjx.config.OAuth2Config.CLIENT_ID;
import static com.zjx.config.OAuth2Config.CLIENT_SECRET;
import static com.zjx.config.OAuth2Config.GRANT_TYPE;
...
public class UserServiceImpl implements UserService {
    ...
    public void addUser(UserDTO userDTO)  {
        ...
        List<User> list=userRepository.findUserByAccount(userDTO.getAccount());
        User userByAccount=null;
        if(list!=null&&list.size()>0){
            userByAccount=list.get(list.size()-1);
        }
    ...
    public ResponseVO login(LoginUserDTO loginUserDTO) {
        ...
        try {
            ...
            try {
                e.printStackTrace();
                return ResponseVO.error(500, "密码错误");
                //此处应该用自定义异常去返回，在这里我就不去具体实现了
        ...
        List<User> list=userRepository.findUserByAccount(loginUserDTO.getAccount());
        User userPO=null;
        if(list!=null&&list.size()>0){
            userPO=list.get(list.size()-1);
        }
